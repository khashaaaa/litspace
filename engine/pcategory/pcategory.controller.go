...
func Store(context *fiber.Ctx) error {
	...
	created, createErr := connector.Query("INSERT INTO pcategory(name) VALUES($1)", data.Name)
...
func ShowAll(context *fiber.Ctx) error {
	...
	rows, rowErr := connector.Query("SELECT mark, name, created, updated FROM pcategory")
...
func ShowSingle(context *fiber.Ctx) error {
	...
	query, queryErr := connector.Query("SELECT mark, name, created, updated FROM pcategory WHERE mark=$1", param)
...
func UpdateSingle(context *fiber.Ctx) error {
	...
	query, queryErr := connector.Prepare("UPDATE pcategory SET name=$1, updated=$2 WHERE mark=$3")
...
func DeleteSingle(context *fiber.Ctx) error {
	...
	query, queryErr := connector.Prepare("DELETE FROM pcategory WHERE mark=$1")
